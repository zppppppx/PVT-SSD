...
def parse_config():
    ...
    parser.add_argument('--pkl_src_dir', type=str, default=None, help="If pkl_src_dir is added, the pkl_files in data_src_dir will be changed!")
...
def eval_command(cfgs):
    eval_sh_path = os.path.join(cfgs.pvt_dir, 'tools/my_new_scripts/eval_waymo.sh')
...
def evaluate(cfgs):
    ...
    if cfgs.pkl_src_dir is not None:
        assert os.path.exists(cfgs.pkl_src_dir)
        segment_file = os.path.join(cfgs.pvt_dir, 'data/waymo/ImageSets/val.txt')
        wu.switch_pkl_files(segment_file, cfgs.pkl_src_dir, cfgs.data_src_dir)
